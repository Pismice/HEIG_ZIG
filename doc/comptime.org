...
In Zig, an expression can have an ~comptime~ to tell the compiler to evaluate the expression at compile-time.
...
The results show that the comptime expression is faster than the runtime one.
...
#+begin_src zig :imports '(std) :main 'no :testsuite yes
  ...
  test "returning a 3D matrix" {
      try std.testing.expect(Matrix3D(f32) == [3][3]f32);
  }
...
In a generic data structure, the ~@This()~ is used to get the type of the data structure because she is anonymous.

Moreover, with generic data structure can have a function that can be called on the structure type and can have a function call on the instance of the structure.
...
Zig can infer the name of a struct when there are created, or a name can be explicitly given :
#+begin_src zig :imports '(std) :main 'no
