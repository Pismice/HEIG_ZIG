...
#+begin_src zig :imports '(std)
    ...
    var list = intList.init(gpa.allocator());
    defer list.deinit();
...
Under the wood, the testing allocator is an instance of the general purpose allocator.
...
#+begin_src zig :imports '(std) :testsuite 'yes
  test "test alloc falling" {
    ...
    var list = intList.init(failingAlloc.allocator());
    defer list.deinit();
