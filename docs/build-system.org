...
The [[https://github.com/hanabi1224/Programming-Language-Benchmarks][Programming Language Benchmark]] [cite:@Hanabi1224ProgrammingLanguageBenchmarksAnother] git repository shows diffrent codes writting in different languages.
...
And finally, we can see that best performing code depend also on the input.
...
The /one billion row challenge/ is a challenge that involves calculing the min, max and average of 1 billion row of measurments (a 12 Gio file). [cite:@BillionRowChallenge]
Everone can submit a code to try to have the better implementation.
Originaly the challenge was for java, but now we can find multiple solutions in many languages.

In this documentation, we tried to see the differences between three implementation: one in C and two in Zig:
...
After each implementation build, we benchmark with hyperfine to have the restults:
...
We can see the differences in performances between C and Zig, but we note that the differences between the best C result and the worst C result has a difference of 6 seconds.
...
In the C implementation, we note that his writer use a custom hashmap to hash the city names.
He use a fast multiplication hash combined with a linear search probing.
